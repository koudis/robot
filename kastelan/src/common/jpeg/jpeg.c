 ...
 *    Copyright (C) 2005-2010  Surveyor Corporation
 *
...
// Planar version of the above
void read_420p_format (JPEG_ENCODER_STRUCTURE *, UINT8 *);
...
void initialization (JPEG_ENCODER_STRUCTURE *jpeg, UINT32 image_format, UINT32 image_width, UINT32 image_height)
{
    ...
    else
    {
        ...
        if (image_format == FOUR_TWO_ZERO_P)
        {
            jpeg->mcu_height = mcu_height = 16;
            jpeg->vertical_mcus = (UINT16) ((image_height + mcu_height - 1) >> 4);

            bytes_per_pixel = 3;
            read_format = read_420p_format;
        }
        else
        {
            jpeg->mcu_height = mcu_height = 8;
...
/* Planar 420 YCbCr format */
void read_420p_format (JPEG_ENCODER_STRUCTURE *jpeg_encoder_structure, UINT8 *input_ptr)
{
    INT32 i, j;
    UINT16 Y1_rows, Y3_rows, Y1_cols, Y2_cols;
